...
with engine.connect() as connection:
    ...
    dim_assets.to_sql(
        ...
        index=False,
        if_exists="replace",
        dtype={"assets_key": sa.INTEGER, "total_assets": sa.BigInteger},
    )
    connection.execute(
        """
    ...
    dim_date.to_sql(
        ...
        index=False,
        if_exists="replace",
        dtype={"date_key": sa.INTEGER, "year": sa.INTEGER},
    )
    connection.execute(
        """
    ...
    dim_deposit_code.to_sql(
        "dim_deposit_code",
        connection,
        index=False,
        if_exists="replace",
        dtype={"deposit_code_key": sa.INTEGER, "deposit_code": sa.VARCHAR(5)},
    )
    connection.execute(
        """
